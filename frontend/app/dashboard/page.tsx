    ...
    <div className="min-h-screen bg-onyx relative">
      ...
      <div className="pt-24 pb-12 px-6 max-w-6xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-semibold text-white tracking-tight">Welcome back, <span className="text-cyber-yellow">{user?.firstName || "User"}</span></h1>
        ...
        {!hasResume ? (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-cyber-yellow/20 flex items-center justify-center">
                  <svg className="w-6 h-6 text-cyber-yellow" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                </div>
                <div>
              ...
              <Button className="rounded-full px-6" onClick={() => router.push("/resumes")}>Upload Resume</Button>
            </div>
          </div>
        ) : (
          <div className="glass-card p-4 rounded-[32px] mb-8 flex items-center justify-between">
            ...
            <Button variant="secondary" className="rounded-full px-4" onClick={() => router.push("/resumes")}>View Resumes</Button>
          </div>
        )}
        <BentoGrid />
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
          <div className="lg:col-span-2"><RecentApplications /></div>
          <div><ActiveAgents /></div>
        </div>
        <div className="mt-12 flex justify-center"><AddJobButton /></div>
      </div>
    </div>
  );
}
